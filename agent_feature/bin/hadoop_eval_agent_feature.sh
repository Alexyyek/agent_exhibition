...
OUTPUT_PATH="${EVAL_AGENT_FEATURE}/${begain_date}_${end_date}"
...
$HADOOP_HOME/bin/hadoop  jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-2.4.1.jar \
    -jobconf mapred.job.name='eval_agent_feature_liuyu' \
    ...
    -file "${RUN_PATH}/bin/agent_feature.py"  \
    ...
    -mapper "./python/bin/python agent_feature.py mapper" \
    -reducer "./python/bin/python agent_feature.py reducer"


if [ $? == 0 ] 
then
    echo "eval agent feature  is success"
else
    echo "eval agent feature is failed"
    exit -1
fi

${HADOOP_HOME}/bin/hadoop fs -getmerge "${OUTPUT_PATH}/part*" "${INNER_DATA_PATH}/${EVAL_AGENT_FEATURE_MERGE_FILENAME}"
if [ $? == 0 ] 
then
    echo "eval agent feature merge is success"
else
    echo "eval agent feature merge  is failed"
    exit -1
fi
${HADOOP_HOME}/bin/hadoop fs -rm -r ${EVAL_AGENT_FEATURE_MERGE_PATH}/${EVAL_AGENT_FEATURE_MERGE_FILENAME}
${HADOOP_HOME}/bin/hadoop fs -put ${INNER_DATA_PATH}/${EVAL_AGENT_FEATURE_MERGE_FILENAME} ${EVAL_AGENT_FEATURE_MERGE_PATH}

if [ $? == 0 ] 
then
    echo "eval agent feature merge is put success"
    exit 0
else
    echo "eval agent feature merge  put  failed"
    exit -1
fi
